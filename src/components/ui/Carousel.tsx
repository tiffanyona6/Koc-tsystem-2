        ...
        <div className="relative w-full h-full max-w-lg mx-auto lg:max-w-none lg:mx-0 rounded-3xl overflow-hidden shadow-xl group">
            <AnimatePresence mode="wait">
                <motion.div
                    ...
                    className="absolute inset-0 w-full h-full bg-gray-100" // Fallback color
                >
                    <Image
                        src={heroImages[currentIndex]}
                        alt={`Targa Slide ${currentIndex + 1}`}
                        fill
                        className="object-cover" // Ensure it covers without white space
                        priority
                    />
                    {/* Dark gradient overlay for text readability if needed, or subtle shadow */}
